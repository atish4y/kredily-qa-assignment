...
AUTO-02: Dashboard Validation (PASS)
AUTO-03: Attendance Anomaly to Correction Request (PASS)
AUTO-04: Approval Reject with Reason (EXECUTED / Caught BUG-009)
AUTO-05: Directory Search and Employee Profile (PASS)
"""
...
def display_automation_report():
    ...
    results = [
        {
            "id": "AUTO-01",
            "name": "Valid Login",
            "status": "BLOCKED / LIMITATION",
            "duration": "N/A",
            "notes": "UiAutomator2 did not expose login fields in fresh session."
        },
        {
            "id": "AUTO-02",
        ...
        {
            "id": "AUTO-04",
            "name": "Approval Reject with Reason",
            "status": "DEFECT CAUGHT",
            "duration": "14.21s",
            "notes": "Caught BUG-009 ('Attendance log not found' on reject)."
        },
        {
            "id": "AUTO-05",
    ...
    for r in results:
        print(f"[{r['id']}] {r['name']:<32} | {r['status']:<20} | {r['duration']}")
        print(f"       -> {r['notes']}\n")

    print("-" * 68)
    print("Summary: 3 PASSED, 1 DEFECT OBSERVED (BUG-009), 1 BLOCKED (DOCUMENTED)")
    print("=" * 68)
